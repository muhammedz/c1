...
<div class="container-fluid">
    <form action="{{ route('admin.corporate.members.update', $member->id) }}" method="POST" enctype="multipart/form-data" id="member-form">
        @csrf
        @method('PUT')
        
        <input type="hidden" name="corporate_category_id" value="{{ $member->corporate_category_id }}">
        <input type="hidden" name="selected_image" id="selected_image" value="{{ $member->image }}">
                            ...
                            <div id="image_preview" style="width: 200px; height: 200px; margin: 0 auto 15px; border-radius: 5px; border: 1px solid #ddd; display: flex; align-items: center; justify-content: center; overflow: hidden; background-color: #f8f9fa;">
                                @if($member->image)
                                    @php
                                        ...
                                        // Eğer URL tam değilse ve storage/ ile başlamıyorsa ekle
                                        if (!filter_var($imagePath, FILTER_VALIDATE_URL) && !Str::startsWith($imagePath, ['/storage', 'storage', 'http', 'https'])) {
