    ...
    <div class="flex flex-col md:flex-row gap-4 md:gap-8">
        ...
        <div class="bg-white rounded-2xl w-full h-48 md:w-1/4 shadow-sm p-6 md:p-8 flex items-center justify-center">
            <div class="text-center">
                <div class="w-16 h-16 md:w-24 md:h-24 mx-auto mb-3 md:mb-4">
                    @if($logoPlans->card1_icon)
                        <i class="fas fa-{{ $logoPlans->card1_icon }} w-full h-full text-[#004d2e]" style="font-size: 4rem;"></i>
                    @else
                ...
                <p class="text-gray-800 font-medium text-sm md:text-base">
                    @if($logoPlans->card1_url)
                        <a href="{{ $logoPlans->card1_url }}">{{ $logoPlans->card1_title ?? 'Yazı Sayfaya Gidecek' }}</a>
                    @else
        ...
        <div class="bg-white rounded-2xl w-full h-48 md:w-1/4 shadow-sm p-6 md:p-8 flex items-center justify-center">
            <div class="text-center">
                @if($logoPlans->card2_image)
                    <img src="/storage/{{ $logoPlans->card2_image }}" alt="{{ $logoPlans->card2_title ?? 'Stratejik Plan' }}" class="w-24 h-24 md:w-32 md:h-32 mx-auto mb-3 md:mb-4 object-contain">
                @else
                ...
                <p class="text-gray-800 font-medium text-sm md:text-base">
                    @if($logoPlans->card2_url)
                        <a href="{{ $logoPlans->card2_url }}">{{ $logoPlans->card2_title ?? 'Stratejik Plan' }}</a>
                    @else
        ...
        <div class="bg-[{{ $logoPlans->logo_bg_color ?? '#004d2e' }}] w-full md:w-2/4 h-48 rounded-2xl p-6 md:p-8 flex items-center justify-center" style="background-color: {{ $logoPlans->logo_bg_color ?? '#004d2e' }}">
            <div class="text-center text-white">
                @if($logoPlans->logo_image)
                    <img src="/storage/{{ $logoPlans->logo_image }}" alt="{{ $logoPlans->logo_title ?? 'Logo' }}" class="max-h-36 max-w-full mx-auto">
                @else
