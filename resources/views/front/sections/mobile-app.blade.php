                ...
                <div class="flex flex-col sm:flex-row gap-4 w-full max-w-xs">
                    ...
                    @else
                        <a href="#" class="flex items-center gap-2 px-4 py-2 border border-[#004d2e] rounded-xl">
                            <svg class="w-6 h-6 text-[#004d2e]" fill="currentColor" viewBox="0 0 24 24">
                                <path d="M18.71,19.5C17.88,20.74 17,21.95 15.66,21.97C14.32,22 13.89,21.18 12.37,21.18C10.84,21.18 10.37,21.95 9.1,22C7.79,22.05 6.8,20.68 5.96,19.47C4.25,17 2.94,12.45 4.7,9.39C5.57,7.87 7.13,6.91 8.82,6.88C10.1,6.86 11.32,7.75 12.11,7.75C12.89,7.75 14.37,6.68 15.92,6.84C16.57,6.87 18.39,7.1 19.56,8.82C19.47,8.88 17.39,10.1 17.41,12.63C17.44,15.65 20.06,16.66 20.09,16.67C20.06,16.74 19.67,18.11 18.71,19.5M13,3.5C13.73,2.67 14.94,2.04 15.94,2C16.07,3.17 15.6,4.35 14.9,5.19C14.21,6.04 13.07,6.7 11.95,6.61C11.8,5.46 12.36,4.26 13,3.5Z" />
                            </svg>
                            <div class="flex flex-col items-start">
                                <span class="text-xs text-[#007b32]">Şimdi İndir</span>
            ...
            <div class="flex flex-col gap-4">
                ...
                @if($mobileAppSettings->link_card_1_title && $mobileAppSettings->link_card_1_url)
                    <a href="{{ $mobileAppSettings->link_card_1_url }}" target="_blank" class="bg-white rounded-2xl p-4 shadow-lg hover:shadow-xl transition-all flex items-center gap-4">
                        <div class="w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0">
                            @if($mobileAppSettings->link_card_1_custom_icon)
                                <img src="{{ asset('storage/' . $mobileAppSettings->link_card_1_custom_icon) }}" alt="İkon" class="w-8 h-8">
                            @else
                                @php
                                    $iconClass = $mobileAppSettings->link_card_1_icon;
                                    // Eğer tam sınıf (fas fa-xxx) şeklinde girilmişse
                                    if (strpos($iconClass, 'fa-') !== false) {
                                        $iconParts = explode('fa-', $iconClass);
                                        if (count($iconParts) > 1) {
                                            $iconName = trim($iconParts[1]);
                                        } else {
                                            $iconName = $iconClass;
                                        }
                                    } else {
                                ...
                                @endphp
                                <i class="fas fa-{{ $iconName }} w-8 h-8 text-[#007b32]" style="font-size: 1.5rem;"></i>
                            @endif
                        </div>
                        <div class="flex-1">
                ...
                @if($mobileAppSettings->link_card_2_title && $mobileAppSettings->link_card_2_url)
                    <a href="{{ $mobileAppSettings->link_card_2_url }}" target="_blank" class="bg-white rounded-2xl p-4 shadow-lg hover:shadow-xl transition-all flex items-center gap-4">
                        <div class="w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0">
                            @if($mobileAppSettings->link_card_2_custom_icon)
                                <img src="{{ asset('storage/' . $mobileAppSettings->link_card_2_custom_icon) }}" alt="İkon" class="w-8 h-8">
                            @else
                                @php
                                    $iconClass = $mobileAppSettings->link_card_2_icon;
                                    // Eğer tam sınıf (fas fa-xxx) şeklinde girilmişse
                                    if (strpos($iconClass, 'fa-') !== false) {
                                        $iconParts = explode('fa-', $iconClass);
                                        if (count($iconParts) > 1) {
                ...
                @if($mobileAppSettings->link_card_3_title && $mobileAppSettings->link_card_3_url)
                    <a href="{{ $mobileAppSettings->link_card_3_url }}" target="_blank" class="bg-white rounded-2xl p-4 shadow-lg hover:shadow-xl transition-all flex items-center gap-4">
                        <div class="w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0">
                            @if($mobileAppSettings->link_card_3_custom_icon)
                                <img src="{{ asset('storage/' . $mobileAppSettings->link_card_3_custom_icon) }}" alt="İkon" class="w-8 h-8">
                            @else
                                @php
                                    $iconClass = $mobileAppSettings->link_card_3_icon;
                                    // Eğer tam sınıf (fas fa-xxx) şeklinde girilmişse
                                    if (strpos($iconClass, 'fa-') !== false) {
                                        $iconParts = explode('fa-', $iconClass);
                                        if (count($iconParts) > 1) {
                                ...
                                @endphp
                                <i class="fas fa-{{ $iconName }} w-8 h-8 text-[#004d2e]" style="font-size: 1.5rem;"></i>
                            @endif
                        </div>
                        <div class="flex-1">
