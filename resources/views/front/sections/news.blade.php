    ...
    <section id="news-section" class="news-section container max-w-7xl mx-auto px-4 py-12">
        <div class="flex flex-col lg:flex-row gap-6">
            ...
            <div class="w-full md:w-[55%] ">
                <div class="swiper newsMainSwiper h-[350px] rounded-2xl overflow-hidden bg-black">
                    <div class="swiper-wrapper">
                        @forelse($headlines as $headline)
                            <div class="swiper-slide relative">
                                ...
                                <div class="absolute bottom-0 left-0 right-0 p-8 z-0">
                                    <div class="inline-block py-1 px-3 bg-black/60 rounded backdrop-blur-sm">
                                        <h3 class="text-2xl font-bold text-white">{{ $headline->title }}</h3>
                                    </div>
                                </div>
                            </div>
                        @empty
                            <div class="swiper-slide relative">
                                ...
                                <div class="absolute bottom-0 left-0 right-0 p-8">
                                    <div class="inline-block py-1 px-3 bg-black/60 rounded backdrop-blur-sm">
                                        <h3 class="text-2xl font-bold text-white">Henüz manşet haber eklenmemiş</h3>
                                    </div>
                                </div>
                            </div>
                        @endforelse
            ...
            <div class="swiper newsThumbsSwiper h-full w-full md:w-[45%] pr-3">
                ...
                <div class="flex justify-between items-center mb-8">
                    <div class="flex items-center gap-4">
                        ...
                        <a href="/haberler" class="text-sm text-[#004d2e] hover:underline">Tüm Haberler</a>
                    </div>
                ...
                <div class="swiper-wrapper grid grid-cols-2 gap-1">
                    @forelse($headlines as $index => $headline)
                        <div class="swiper-slide !h-[120px] cursor-pointer">
                            <a href="{{ route('news.show', $headline->slug) }}" class="w-full h-full transparent rounded-2xl overflow-hidden transition-all block border border-gray-100">
                                <div class="p-4 h-full flex items-center">
                                    <h3 class="text-sm font-medium text-gray-800 line-clamp-3">{{ $headline->title }}</h3>
                                </div>
                            </a>
                        </div>
                    @empty
                        <div class="swiper-slide !h-[120px] cursor-pointer">
                            <div class="w-full h-full transparent rounded-2xl overflow-hidden transition-all block border border-gray-100">
                                <div class="p-4 h-full flex items-center">
                                    <h3 class="text-sm font-medium text-gray-800 line-clamp-3">Manşet haber bulunamadı</h3>
                                </div>
                            </div>
                        </div>
                    @endforelse
